...
export async function generateColoringPage(promptText: string): Promise<string> {
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        const fullPrompt = `A simple, bold, black and white coloring book page for a 5-year-old of a ${promptText}.`;
...
export async function generateStory(promptText: string): Promise<string> {
    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        const fullPrompt = `Write a short, fun, and imaginative story for a 5-year-old child about a ${promptText}. The story must be 3 to 4 sentences.`;
        const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash',
            contents: fullPrompt,
        });
